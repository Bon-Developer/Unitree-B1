...
void Base::LowLevelControl() {
    ...
    if(motiontime / 500 == 0){ // 1초에 한번씩 출력하도록
        ...
        printf("FR_1 : %d  %f  %f  %f\n", elasped_time, base_low_state.motorState[UT::FR_1].q, base_low_state.motorState[UT::FR_1].dq, base_low_state.motorState[UT::FR_1].tau);
        printf("\n\n");
    }
    ...
    {
        sin_count++;
        float torque = (0 - base_low_state.motorState[FR_1].q) * 10.0f + (0 - base_low_state.motorState[FR_1].dq) * 1.0f;
        
        if(torque > 5.0f) torque = 5.0f;
        if(torque < -5.0f) torque = -5.0f;


        robot_low_cmd.motorCmd[FR_1].q = PosStopF;
        robot_low_cmd.motorCmd[FR_1].dq = VelStopF;
        robot_low_cmd.motorCmd[FR_1].Kp = 0;
        robot_low_cmd.motorCmd[FR_1].Kd = 0;
        robot_low_cmd.motorCmd[FR_1].tau = torque;

        printf("robot_low_cmd.motorCmd[UT::FR_1].q : %f\n", robot_low_cmd.motorCmd[UT::FR_1].q);
        printf("robot_low_cmd.motorCmd[UT::FR_1].dq : %f\n", robot_low_cmd.motorCmd[UT::FR_1].dq);
        printf("robot_low_cmd.motorCmd[UT::FR_1].tau : %f\n", robot_low_cmd.motorCmd[UT::FR_1].tau);
    }
