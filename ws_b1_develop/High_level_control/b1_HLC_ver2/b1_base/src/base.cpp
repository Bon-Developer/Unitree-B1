...
Base::Base(UT::HighCmd cmd, UT::HighState state, ros::NodeHandle *nh, ros::NodeHandle *p_nh): robot_safety_(UT::LeggedType::B1) {
    ...
    robot_high_cmd.bodyHeight      = 0.0; //default 0.28m
    robot_high_cmd.footRaiseHeight = 0.0; //default 0.08m

    //Subscriber 
    //cmd_vel 토픽, &Base::cmdVelCallback - Base 클래스의 멤버 함수 cmdVelCallback 사용, this 포인터: Base 클래스의 인스턴스 참조
    ...
    //Service server
    //set_mode_: 서비스 이름 
    set_mode_ = p_nh_->advertiseService("set_mode_", &Base::, this);

    //string 클래스 타입의 변수 ip_string 선언
    std::string ip_string;

    //매개변수 서버 조회, 기본값: 192.168.123.220
    p_nh_->param<std::string>("target_ip", ip_string, "192.168.123.220");

    //ip_string에 저장된 IP 주소 문자열을 udp_ip_ 배열에 복사 
...
Base::Base(UT::LowCmd  cmd, UT::LowState  state, ros::NodeHandle *nh, ros::NodeHandle *p_nh): robot_safety_(UT::LeggedType::B1) {
    ...
    for(int i = 0; i<12; i++) {
        robot_low_cmd.motorCmd[i].mode = 0x0A; //10진수로 10
    ...
    //low level contro에서는 192.168.123.10
